...
export default async function Dashboard() {
  ...
  console.log("Page session:", JSON.stringify(session?.user));

  const [posts, userLikes, saved] = await Promise.all([
    ...
    <div className="space-y-12">
      ...
      <section className="text-center py-16 px-4 bg-surface rounded-3xl shadow-sm border border-border">
        ...
        <p className="text-xl text-foreground opacity-70 max-w-2xl mx-auto mb-8">
          Discover stories, thinking, and expertise from writers on any topic. A
          modern, minimalist approach to reading.
        </p>
        <Link
          href="/editor"
              ...
              >
                ...
                <div className="flex flex-col grow p-6">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-border">
                    ...
                    <SaveButton
                      postId={post.id}
                      isSaved={saved.some((save) => save.postId === post.id)}
                    />
                  </div>
                </div>
              </article>
